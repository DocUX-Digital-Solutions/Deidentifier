...
class FieldSpoofer:
    ...
    def create(cls,
               tally: Dict[str, int],
               min_count: int = 1):
        tally = {k: v for k, v in tally.items() if v >= min_count}
        forms = sorted(list(tally.keys()))
        lc_forms = [f.lower() for f in forms]
        assert len(set(forms)) == len(set(lc_forms)) == len(tally)
        return cls(forms,
                   lc_forms,
                   [tally[k] for k in forms])

    def give_random_item(self,
                         exclude_form: Optional[str]) -> str:
        lc_ind = None
        if exclude_form is not None:
            try:
                lc_ind = self.lc_forms.index(exclude_form.lower())
            except ValueError:
                pass

        if lc_ind is None:
            use_forms = self.forms
            use_weights = self.weights
        else:
            use_forms = self.forms[:lc_ind] + self.forms[(1+lc_ind):]
            use_weights = self.forms[:lc_ind] + self.forms[(1+lc_ind):]

        return random.choices(use_forms, weights=use_weights, k=1)[0]
...
class VendorSpoofer(TypeSpoofer):
    ...
    def create(cls,
               ...
               _max_tries: int = 5):
        spoofer = FieldSpoofer.create(vendor_tally, min_count)
        if not allow_equal_output and len(spoofer.forms) <= 1:
            raise ValueError
    ...
    def spoof_value(self,
                    old_value: str,
                    count: int = 1) -> str:
        return count * [self.spoofer.give_random_item(old_value if not self.allow_equal_output else None)]
...
class HospitalSpoofer(TypeSpoofer):
    ...
    def create(cls,
        ...
        # Would need link each surface form to an actual institution
        if allow_equal_output is False:
            raise NotImplementedError
        ...
        return cls(FieldSpoofer(sources),
                   FieldSpoofer(department_tally),
                   format_weights,
    ...
    def spoof_value(self,
        ...
        self.hospital_spoofer.give_random_item(old_hospital)
...
class DateSpoofer(TypeSpoofer):
    ...
    def spoof_value(self,
                    old_value: str,
                    count: int =1) -> List[str]:
        parsed_date, parsed_format = self.date_converter.string_to_date_and_format(old_value)
...
class NameSpoofer(TypeSpoofer):
    @classmethod
    def spoof_value(self, given, middle, surname, credential) -> str:
        raise NotImplementedError
...
class HealthCareWorkerSpoofer(NameSpoofer):
    ...
    def create(cls,
        ...
        return cls(FieldSpoofer.create(surname_tally, min_surname_count),
                   FieldSpoofer.create(given_tally, min_given_count),
...
Returns the number of names per name format
    ...
    def generate_random_name(self, given, middle, surname, credential) -> Tuple[str, str, str, str]:
        return (self.given_spoofer.give_random_item(given),
                self.given_spoofer.give_random_item(middle),
                self.surname_spoofer.give_random_item(surname),
                self.credential_spoofer.give_random_item(credential))

    def spoof_value(self,
                    given, middle, surname, credential,
                    count: int = 1) -> List[str]:
        new_name_fields = self.generate_random_name(given, middle, surname, credential)
...
class PatientSpoofer(NameSpoofer):
    ...
    def create(cls,
        ...
        return cls(FieldSpoofer.create(surname_tally, min_surname_count),
                   FieldSpoofer.create(given_tally, min_given_count),
                   FieldSpoofer.create(PatientSpoofer._create_honorific_tally()),
                   _max_tries=max_tries)
...
class Spoofer:
    ...
    type_spoofers: List[TypeSpoofer]
    error_prob = 1 / 500
    ...
    def create(cls,
        ...
        type_spoofers: List[TypeSpoofer] = [
            VendorSpoofer.create(value_tallies['VENDOR']),
            DateSpoofer.create(min_date=all_dates[0],
                               max_date=all_dates[-1],
                               date_tally=date_tally,
                               date_format_tally=date_format_tally),
            HealthCareWorkerSpoofer.create(hcw_surname_tally,
                                           hcw_given_tally,
                                           hcw_name_format_tally),
            PatientSpoofer.create(patient_surname_tally,
                                  patient_given_tally),
            HospitalSpoofer.create(),
    ...
    dict_generate_phi = {
        ...
        "DATES": (
            lambda date, constraint: generate_date(
                date,
                distribution_probabilities_date, # weights for date options
                convert_date_to_string, # code
                convert_string_to_date, # code
                min_date, # Of the input, as a date, not a string
                max_date, # ditto
                store_dates=None,
                constraint=constraint,
            )
